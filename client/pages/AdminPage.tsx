...
const AdminPage = () => {
  ...
  const fetchServices = async (search?: string) => {
    try {
      ...
      if (data.success) {
        ...
        setFilteredData(prev => ({ ...prev, services: data.data || [] }));
        setStats(prev => ({ ...prev, totalServices: data.data?.length || 0, activeServices: data.data?.filter((s: Service) => s.is_active).length || 0 }));
      }
    } catch (error) {
  ...
  const fetchProducts = async (search?: string) => {
    try {
      ...
      if (data.success) {
        ...
        setFilteredData(prev => ({ ...prev, products: data.data || [] }));
  ...
  const fetchQuotes = async (search?: string) => {
    try {
      ...
      if (data.success) {
        setQuotes(data.data || []);
        setFilteredData(prev => ({ ...prev, quotes: data.data || [] }));
        const recentQuotes = data.data?.filter((q: Quote) => {
          const quoteDate = new Date(q.created_at);
          const weekAgo = new Date();
          weekAgo.setDate(weekAgo.getDate() - 7);
          return quoteDate > weekAgo;
        }).length || 0;
        setStats(prev => ({ ...prev, totalQuotes: data.data?.length || 0, recentQuotes }));
      }
    } catch (error) {
  ...
  const fetchUsers = async (search?: string) => {
    try {
      ...
      if (data.success) {
        setUsers(data.data || []);
        setFilteredData(prev => ({ ...prev, users: data.data || [] }));
  ...
  const fetchTestimonials = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/testimonials?activeOnly=false');
      const data = await response.json();
      if (data.success) {
  ...
  const fetchBlogPosts = async (search?: string) => {
    try {
      setLoading(true);
      const url = search ? `/api/blog?search=${encodeURIComponent(search)}` : '/api/blog';
      const response = await fetch(url);
      const data = await response.json();
      if (data.success) {
        ...
        setStats(prev => ({
          ...
          publishedPosts: data.data?.filter((post: BlogPost) => post.status === 'published').length || 0
        }));
      }
    } catch (error) {
  ...
  // Fetch AWStats analytics data
  const fetchAnalytics = async () => {
    try {
      const response = await fetch('/api/analytics/awstats');
      const data = await response.json();
      if (data.success) {
        setAnalyticsData(data.data);
      } else {
        setAnalyticsData(null);
        toast.error(data.error || 'Failed to load AWStats data');
      }
    } catch (error) {
      console.error('Error fetching AWStats:', error);
      toast.error('Failed to load AWStats data');
    }
  };
    ...
    if (window.confirm("Are you sure you want to delete this item?")) {
      try {
        ...
        if (data.success) {
          ...
          toast.error(data.error || `Failed to delete ${type.slice(0, -1)}`);
        }
      } catch (error) {
  ...
  const handleSave = async (type: string, data: any) => {
    try {
      ...
      if (editingItem) {
        ...
        response = await fetch(`/api/${type}/${editingItem.id}`, {
          method: 'PUT',
          headers: {
        ...
        // Create new item
        // Map types to correct API resource names when they differ
        const resource = type === 'contacts' ? 'contact' : type;
      ...
      if (result.success) {
        ...
        // Show backend validation error details if present
        if (result.details) {
          toast.error(
          ...
          toast.error(result.error || `Failed to ${editingItem ? 'update' : 'create'} ${type.slice(0, -1)}`);
        }
      }
    } catch (error) {
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <ModernButton variant="ghost" size="sm" onClick={async () => {
                      await fetch(`/api/contact/${msg.id}/read`, { method: 'PUT' });
                      fetchContacts();
                    }}>
                      <Eye className="w-4 h-4" />
                    </ModernButton>
                    <ModernButton variant="ghost" size="sm" onClick={async () => {
                      await fetch(`/api/contact/${msg.id}/replied`, { method: 'PUT' });
                      fetchContacts();
                    }}>
            ...
            <ModernCard key={t.id}>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <ModernButton variant="ghost" size="sm" onClick={async () => {
                      await fetch(`/api/testimonials/${t.id}/featured`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ featured: !t.is_featured })});
                      fetchTestimonials();
                    }}>
                  ...
                  <div className="flex items-center gap-2">
                    {post.status === 'draft' && (
                      <ModernButton 
                        ...
                        size="sm" 
                        onClick={async () => {
                          await fetch(`/api/blog/${post.id}/publish`, { method: 'PUT' });
                          fetchBlogPosts();
                        }}
                      >
                        <Eye className="w-4 h-4" />
  ...
  // AWStats Analytics Tab
  const renderAnalytics = () => (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-slate-900">AWStats Analytics</h2>
      </div>
      <ModernCard>
        <CardContent className="p-6">
          {analyticsData ? (
            ...
            <div className="h-32 flex items-center justify-center text-slate-500">No AWStats data available.</div>
          )}
        </CardContent>
      </ModernCard>
    </div>
    ...
    if (type === 'products') {
      ...
      if (imageFile) {
        try {
          ...
          // Always send entity_type as string
          imgForm.append('entity_type', 'product');
          ...
          // Do NOT set headers, let browser set Content-Type for FormData
          ...
          if (imgData.success && imgData.data?.url) {
            dataToSend.image_url = imgData.data.url;
          } else {
            let details = '';
            if (imgData.details) {
              if (Array.isArray(imgData.details)) {
                details = imgData.details.map((d: any) => d.message).join('; ');
              } else {
                details = JSON.stringify(imgData.details);
              }
            }
            toast.error('Image upload failed: ' + (imgData.error || 'Unknown error') + (details ? ' | ' + details : ''));
            return;
          }
        } catch (err) {
