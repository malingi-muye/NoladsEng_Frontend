...
export default function BlogAdmin() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const publishedPosts = postsRes.success ? postsRes.data.filter(p => p.is_published) : [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        const imageRes = await fetch('/api/images/upload', {
          method: 'POST',
          body: formData
        });
      ...
      if (activeTab === 'posts') {
        if (editingItem) {
          response = await api.blog.update(editingItem.id, data);
        } else {
          response = await api.blog.create(data);
        }
      } else if (activeTab === 'categories') {
        if (editingItem) {
          response = await api.blog.updateCategory(editingItem.id, data);
        } else {
          response = await api.blog.createCategory(data);
        }
      }
