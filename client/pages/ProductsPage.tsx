...
function ProductsPage() {
      ...
      if (editingProduct) {
        ...
        if (response.success) {
          setProducts(prev => prev.map(p => 
            p.id === editingProduct.id ? mapServerToClientProduct(response.data) : p
          ));
        }
      } else {
        const response = await api.products.create(productData);
        if (response.success) {
          setProducts(prev => [mapServerToClientProduct(response.data), ...prev]);
        }
      }
  ...
  const mapServerToClientProduct = (product: ServerProduct): ProductExtended => ({
    ...product,
    image: product.image_url || '',
    inStock: product.stock_quantity > 0,
    tags: product.specifications ? JSON.parse(product.specifications).tags || [] : [],
    rating: 0,
    reviews: 0,
  });
  ...
  const fetchProductsData = async () => {
    ...
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        const extendedProducts = response.data.map(mapServerToClientProduct);
