...
export function QuoteRequestsAdmin() {
  ...
  const fetchQuoteRequests = async () => {
    try {
      const response = await fetch('/api/admin/quote-requests.php');
  ...
  const updateQuoteRequest = async (id: number, data: Partial<QuoteRequest>) => {
    try {
      const response = await fetch('/api/admin/update-quote-request.php', {
        method: 'POST',
  ...
  const handleViewDetails = async (request: QuoteRequest) => {
    try {
      const response = await fetch(`/api/admin/get-quote-request.php?id=${request.id}`);
      if (!response.ok) throw new Error('Failed to fetch quote request details');
      const data = await response.json();
